...
import type { GoogleMailAccountConfig } from "./types.js";
...
function buildAuthKey(account: GoogleMailAccountConfig): string {
  ...
  if (account.serviceAccountCredentials) {
    const hash = createHash("sha256")
      .update(JSON.stringify(account.serviceAccountCredentials))
      .digest("hex")
      .slice(0, 16);
    return `sa-inline:${hash}`;
  }
  return "none";
}
...
/**
 * Returns an authenticated client suitable for Gmail API calls.
 *
 * Priority:
 ...
 */
function getAuthInstance(account: GoogleMailAccountConfig): AuthInstance {
  ...
  const auth = createServiceAccountAuth({ scopes: GMAIL_SCOPES });
  authCache.set(account.accountId, { key, auth });
  evictOldest();
  return auth;
}
...
export function clearGmailAuthCache(accountId: string): void {
  authCache.delete(accountId);
}

export { GMAIL_SCOPES as GMAIL_SCOPES_LIST } from "../shared/auth.js";
